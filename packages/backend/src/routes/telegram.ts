...
router.post('/owner-reject', async (req, res) => {
  try {
    ...
    const room = await db.query.rooms.findFirst({ where: eq(rooms.id, viewing.roomId) })
    if (!lead || !room) return res.status(404).json({ error: 'Lead or room not found' })
    await db.update(viewings).set({ status: 'owner_rejected', ownerRejectionReason: reason, task: null, updatedAt: new Date() }).where(eq(viewings.id, viewing.id))
...
router.post('/command', async (req, res) => {
  try {
    ...
    // Find existing viewing for this lead+room, or CREATE one (for multi-room viewing)
    let viewing = await db.query.viewings.findFirst({
      where: and(eq(viewings.leadId, lead.id), eq(viewings.roomId, room.id))
    })
    if (!viewing) {
      // Auto-create viewing record when agent schedules viewing for a new room
