import { Router } from 'express'
  ...
  router.post('/owner-confirm', async (req, res) => {
    try {
      ...
      await db.update(viewings).set({ status: 'owner_confirmed', task: 'Invite view house', taskChecked: false, updatedAt: new Date() }).where(eq(viewings.id, viewingId))
  ...
  router.post('/owner-reject', async (req, res) => {
    try {
      ...
      const room = await db.query.rooms.findFirst({ where: eq(rooms.id, viewing.roomId) })
      if (!lead || !room) return res.status(404).json({ error: 'Lead or room not found' })
      await db.update(viewings).set({ status: 'owner_rejected', ownerRejectionReason: reason, task: null, taskChecked: false, updatedAt: new Date() }).where(eq(viewings.id, viewing.id))
    ...
    try {
      ...
      if (type === 'pay_rent') {
        ...
        await db.update(viewings).set({
          status: 'moved_in', task: 'Pay Rent', taskChecked: false,
      ...
      let viewing = await db.query.viewings.findFirst({
        where: and(eq(viewings.leadId, lead.id), eq(viewings.roomId, room.id))
      })
      if (!viewing) {
        const [newViewing] = await db.insert(viewings).values({
          leadId: lead.id,
          roomId: room.id,
          status: 'owner_confirmed',
          task: 'Invite view house',
          taskChecked: false,
        }).returning()
        viewing = newViewing
      }
      ...
      if (type === 'view_house_scheduled') {
        ...
        await db.update(viewings).set({ status: 'viewing_scheduled', scheduledDate: d, task: `View house ${dateStr}`, taskChecked: false, updatedAt: new Date() }).where(eq(viewings.id, viewing.id))
      }
      else if (type === 'view_house_done') {
        await db.update(viewings).set({ status: 'viewing_done', task: 'Book?', taskChecked: false, updatedAt: new Date() }).where(eq(viewings.id, viewing.id))
      }
      else if (type === 'booked' || type === 'paid') {
        let update: any = { status: type, taskChecked: false, updatedAt: new Date() }
        if (dateStr) {
          const parts = (dateStr as string).split('/')
