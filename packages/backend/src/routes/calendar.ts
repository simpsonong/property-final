import { Router } from 'express'
  ...
  router.get('/:year/:month', async (req, res) => {
    try {
      ...
      // VIEW HOUSE events (viewing_scheduled with a date this month)
      ...
      const viewHouseEvents = await Promise.all(viewHouseRows.map(async (v) => {
        ...
        return {
          date: v.scheduledDate, leadId: v.leadId, viewingId: v.id,
          leadName: lead?.name ?? '', roomAddress: room?.address ?? '',
          taskChecked: v.taskChecked ?? false,
      ...
      const moveInEvents = await Promise.all(moveInRows.map(async (v) => {
        ...
        return {
          date: v.confirmedMoveInDate, leadId: v.leadId, viewingId: v.id,
          leadName: lead?.name ?? '', roomAddress: room?.address ?? '',
          status: v.status, taskChecked: v.taskChecked ?? false,
          waLink: lead ? buildWaLink(lead.phone, msg) : null,
        }
      }))
      ...
      const payRentEvents = await Promise.all(rentRows.map(async (r) => {
        ...
        // For rent payments, use viewingId to allow check toggle
        const viewing = await db.query.viewings.findFirst({ where: eq(viewings.id, r.viewingId) })
        return {
          date: r.dueDate, leadId: r.leadId, viewingId: r.viewingId,
          leadName: lead?.name ?? '', roomAddress: room?.address ?? '',
          taskChecked: viewing?.taskChecked ?? false,
          waLink: lead ? buildWaLink(lead.phone, msg) : null,
        }
      }))
